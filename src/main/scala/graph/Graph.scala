...
object Graph {
   ...
   * heuristic: Function that estimates the cost of going from one node to another.
   *            Has to be admissible (no overestimation) and consistent
   * costFunc:  Function to get the cost of moving from one node to another.
   *            It is guaranteed to only be called on neighbors
   *
   * R= fringe +eturns the list of nodes to traverse.
   */
  def astar[T](start: T,
               goal: T,
               neighbors: T => List[T],
               heuristic: (T, T) => Float,
               costFunc:  (T, T) => Float): List[T] = {
    ...
    var fringe: Set[(Float, List[T])] = Set((0f,start::Nil))
    var closed: Set[T] = Set[T]()
  ...
  /** Mutable A*. Same as {{{astar}}} but attempts to use mutable structures for
    * performance. Should be exactly the same
    */
  def mAstar[T](start: T,
                goal: T,
                neighbors: T => List[T],
                heuristic: (T, T) => Float,
                costFunc:  (T, T) => Float): List[T] = {
    ...
    val fringe = mutable.Set((0f, start::Nil))
